...
class FineTuningDataset(IDataset):
    ...
    def get_io_tensors(self, conv: Conversation) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        #                      User: Q R S T U
        #                      Bot:  V W X Y Z
        # Input Structure:     [SYSTEM] K  L  M  N  O  [USER]  A B  C  D  E ... [BOT]  V  W  X  Y    Z    $ $ $
        ...
        for exchange in conv.exchanges:
            input_text = exchange["input"]
            output_text = exchange["output"]
            ...
            last_bot_token_idx = len(input_ids) + len(input_token_ids) + 1
            input_ids.extend([
                self.user_token,
                *input_token_ids,
                self.bot_token,
